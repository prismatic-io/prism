...
describe("profile context", () => {
  ...
  it("uses the default URL without a profile or environment override", async () => {
    expect(await runWithCommandContext(createCommandContext(), getPrismaticUrl)).toBe(
      DEFAULT_PRISMATIC_URL,
    );
  });
  ...
  it("rejects a URL that conflicts with stored profile credentials", async () => {
    ...
    await expect(runWithCommandContext(createCommandContext(), getPrismaticUrl)).rejects.toThrow(
      /does not match profile 'default'/,
    );
  });
  ...
  it("keeps a profile tenant with its profile credentials", async () => {
    ...
    expect((await runWithCommandContext(createCommandContext(), getAuthContext)).tenantId).toBe(
      "stored-tenant",
    );
  ...
  it("does not borrow a profile tenant for environment credentials", async () => {
    ...
    expect(
      (await runWithCommandContext(createCommandContext(), getAuthContext)).tenantId,
    ).toBeUndefined();
  });
  ...
  it("uses the whole environment credential set instead of mixing it with a profile", async () => {
    ...
    expect(await runWithCommandContext(createCommandContext(), getAuthContext)).toEqual({
      source: "environment",
  ...
  it("forces profile credentials for profile-management auth flows", async () => {
    ...
    expect(
      await runWithCommandContext(createCommandContext({ profileOnly: true }), getAuthContext),
    ).toEqual({
      source: "profile",
