...
export default class ListenCommand extends PrismaticBaseCommand {
  ...
  static flags = {
    "integration-id": Flags.string({
      char: "i",
      description: "ID of the integration containing the flow to listen on.",
      required: true,
    }),
    "flow-id": Flags.string({
      char: "f",
      description: "ID of the flow to listen on. If not provided, you will be prompted to select.",
    }),
    output: Flags.string({
      char: "o",
  ...
  async run() {
    ...
    if (!flowId) {
      const selectedFlow = await selectFlowPrompt(integrationId, {
        message: "Select the flow to listen on:",
      });
    ...
    try {
      const execution = await this.pollForExecution(flowId, timeout);
  ...
  /**
   * Toggle listening mode on an integration
   */
  ...
  /**
   * Poll for new executions on the flow
   */
  private async pollForExecution(flowId: string, timeout: number): Promise<ExecutionResult | null> {
    ...
    while (true) {
      ...
      if (result.executionResults.nodes.length > 0) {
        ...
        this.log("\nExecution complete.");
        this.log(`  Started at: ${execution.startedAt}`);
        this.log(`  Ended at: ${execution.endedAt}`);
        return execution;
      }
    }
  }
