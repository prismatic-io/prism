...
export const buildConnections = (
  ...
  if (scheme.type === "oauth2") {
    if ("authorizationCode" in scheme.flows) {
      ...
      const usesScopes = authCodeFlow?.scopes && Object.keys(authCodeFlow.scopes).length > 0;
      const connection = stripUndefined<Connection>({
        orderPriority: 0,
        key: camelCase(key),
    ...
    if ("clientCredentials" in scheme.flows) {
      ...
      const usesScopes = clientCredsFlow?.scopes && Object.keys(clientCredsFlow.scopes).length > 0;
      const connection = stripUndefined<Connection>({
        orderPriority: 10,
        key: camelCase(key),
