...
import {
  type Configuration as Credentials,
  deleteProfile,
  replaceCredentials,
  writeProfile,
} from "./config.js";
...
export const login = async (props?: { url: boolean; profileName?: string }) => {
  ...
  const { profileName, configPath, url: prismaticUrl } = target;
  ...
  await writeProfile(profileName, { ...initialAuth, prismaticUrl }, configPath);
  ...
  if (selectedTenantId) {
    ...
    await writeProfile(profileName, { ...tenantAuth, prismaticUrl }, configPath);
  } else if (initialTenantSuspended) {
    await deleteProfile(profileName, configPath);
  }
...
const saveRefreshedCredentials = async (
  ...
  if (
    !target.profile ||
    !(await replaceCredentials(target.profileName, target.profile, credentials, target.configPath))
  ) {
    throw new Error(
...
export const revokeRefreshToken = async (): Promise<AuthContext["source"]> => {
  ...
  if (authContext.source === "profile" && authContext.profileName) {
    await deleteProfile(authContext.profileName, authContext.configPath);
  }
