...
const buildComponent = (api: OpenAPI.Document): Component => {
  const component = stripUndefined<Component>({
    display: {
      ...
      description:
        api.info.description ?? `Generated component for ${api.info.title} ${api.info.version}`,
      iconPath: "icon.png",
    },
  });
  return component;
...
export const read = async (filePath: string): Promise<Result> => {
  ...
  const connections =
    "components" in api
      ? Object.entries(api.components?.securitySchemes ?? {}).reduce<Connection[]>(
          (result, [key, scheme]) => [...result, ...buildConnections(key, scheme)],
          [],
        )
      : [];
