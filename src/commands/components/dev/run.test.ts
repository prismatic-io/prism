...
it.each([
...
])("preserves connection metadata %j in native subprocess events", async (meta) => {
  ...
  try {
    await writeFile(
      script,
      'const connection = JSON.parse(process.env.PRISMATIC_CONNECTION_VALUE); console.log(connection.fields.token + ":" + connection.session); setTimeout(() => console.error("diagnostic"), 50);',
    );
    ...
    expect(events).toEqual([
      { type: "stdout", data: "test-token:session-value\n" },
